...
import { UnifiedDataPoint, GenesysCredentials, AgentPerformance, WrapUpData, BranchData, CallerData, ReasonBranchData, CustomerConversation, InteractionRecord } from './types';
import { 
  GENESYS_ORG_DEFAULT, 
...
const App: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setIsFetching(true);
    try {
      const metrics = await fetchRealtimeMetrics(creds, activeQueueId, actualRegion, selectedDate, activeTab === 'daily' ? dateTo : selectedDate);
      ...
      const recs = await fetchRecentInteractions(creds, activeQueueId, actualRegion, 
        recFromTime && recToTime ? selectedDate : undefined,
        recFromTime || undefined,
        recToTime || undefined
      );
      setRecordings(recs);
  ...
  }, [creds, activeQueueId, actualRegion, selectedDate, dateTo, activeTab, recFromTime, recToTime]);
  ...
  const metrics = useMemo(() => {
    ...
    const max = { mos: Math.max(...intervals.map(i => i.mos)), sl: Math.max(...intervals.map(i => i.sl)), offered: Math.max(...intervals.map(i => i.offered)), answered: Math.max(...intervals.map(i => i.answered)), abandoned: Math.max(...intervals.map(i => i.abandoned)), agents: Math.max(...intervals.map(i => i.agents)), aht: Math.max(...intervals.map(i => i.aht)), avgCalls: Math.max(...intervals.map(i => i.avgCalls)) };
    const min = { mos: Math.min(...intervals.filter(i => i.mos > 0).map(i => i.mos)), sl: Math.min(...intervals.filter(i => i.sl > 0).map(i => i.sl)), offered: Math.min(...intervals.map(i => i.offered)), answered: Math.min(...intervals.map(i => i.answered)), abandoned: Math.min(...intervals.map(i => i.abandoned)), agents: Math.min(...intervals.map(i => i.agents)), aht: Math.min(...intervals.filter(i => i.aht > 0).map(i => i.aht)), avgCalls: Math.min(...intervals.filter(i => i.avgCalls > 0).map(i => i.avgCalls)) };
  ...
  const renderTabButton = (st: SubTabType) => {
    ...
    return (
      <button 
        ...
        className={`flex-1 min-w-[120px] px-4 py-3 text-[10px] font-black tracking-wider uppercase transition-all border border-slate-300 rounded-t-xl ${isActive ? 'bg-slate-50 border-b-transparent text-emerald-600 shadow-[0_-2px_4px_rgba(0,0,0,0.03)]' : 'bg-slate-200 text-slate-500 hover:bg-slate-300 hover:text-slate-700'}`}
      >
        {subTabLabels[st]}
      </button>
    );
      ...
      <div className="sticky top-0 z-40 bg-white shadow-sm">
        <header className="px-6 py-4 flex items-center justify-between gap-4 border-b border-slate-100">
          <div className="flex items-center gap-4">
            <img src="/mawsool.png" alt="Mawsool" className="w-12 h-12 object-contain rounded-xl shadow-md" />
            <div>
          ...
          <div className="flex items-center gap-3">
            {deferredPrompt && (
              <button 
                onClick={installPWA}
                className="flex items-center gap-2 px-3 py-2 bg-emerald-500 text-white rounded-lg shadow-lg shadow-emerald-500/30 active:scale-95 transition-all"
              >
                <i className="fa-solid fa-mobile-screen-button"></i>
                <span className="text-[10px] font-black uppercase tracking-widest hidden sm:inline">Install App</span>
              </button>
            )}
        ...
        {isConfigOpen ? (
          <div className="max-w-xl mx-auto py-12">
            <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-200">
              <div className="flex flex-col items-center mb-8">
                <img src="/mawsool.png" alt="Mawsool" className="w-20 h-20 object-contain rounded-2xl mb-4 shadow-xl shadow-slate-200/50" />
              ...
              <form onSubmit={handleConnect} className="space-y-6">
                <div className="space-y-4">
                  <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Client ID</label><input type="text" value={formData.clientId} onChange={e => setFormData({...formData, clientId: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all" /></div>
                  <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Client Secret</label><input type="password" value={formData.clientSecret} onChange={e => setFormData({...formData, clientSecret: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all" /></div>
                  <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Queue Identity</label><input type="text" value={formData.queueName} onChange={e => setFormData({...formData, queueName: e.target.value})} className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 outline-none focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 transition-all" /></div>
                </div>

                <button type="submit" className="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-xl transition-all shadow-lg shadow-emerald-500/20 active:scale-[0.98] flex items-center justify-center gap-3"><i className="fa-solid fa-plug"></i><span className="uppercase tracking-widest">Connect Mawsool</span></button>
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            ...
            {activeSubTab === 'recordings' && (
              <RecordingsView 
                recordings={recordings} 
                fromTime={recFromTime} 
                toTime={recToTime} 
                setFromTime={setRecFromTime} 
                setToTime={setRecToTime} 
                onRefresh={fetchData} 
              />
            )}
            ...
            {activeSubTab === 'aiforensics' && (
              <div className="space-y-8 py-10 flex flex-col items-center">
                 ...
                 <h3 className="text-lg font-black uppercase text-slate-900 tracking-tight">AI Quality Forensics</h3>
                 <p className="text-sm font-bold text-slate-400 uppercase tracking-widest text-center max-w-md">
                    Connect Gemini 3 Pro to generate deep-dive forensic hypotheses on voice quality and traffic patterns.
                 </p>
                 <button disabled className="px-10 py-4 bg-slate-100 text-slate-400 text-[10px] font-black uppercase tracking-widest rounded-2xl border border-slate-200 shadow-inner">
                    Intelligence Layer Restricted
                 </button>
              </div>
            )}
          </div>
